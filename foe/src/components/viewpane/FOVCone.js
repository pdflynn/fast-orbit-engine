...
const FOVCone = ({ ts, xs, ys, zs, r_cone, h_cone, delta_h, color }) => {
    ...
    useFrame(() => {
        ...
        if (timeInOrbit >= nextTime) {
            ...
            coneMesh.current.geometry = new THREE.ConeBufferGeometry(r_cone[currentIndx], h_cone[currentIndx], 512);
            ...
            if (currentIndx>=ts.length) {
                ...
                coneMesh.current.geometry = new THREE.ConeBufferGeometry(r_cone[0], h_cone[0], 512);
